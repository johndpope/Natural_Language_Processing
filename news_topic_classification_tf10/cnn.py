...
class topicCNN(object):
    def __init__(self, vocabulary_size, embedding_size, filter_size, filter_num, max_news_size, topic_size):
        ...
        res_after_pool=[]
        for index,temp_filter_size in enumerate(tf.unstack(filter_size)):
            with tf.name_scope('convolution_pool_with_filter_size_%s' % index):
                filter_shape=[temp_filter_size.eval(),embedding_size,1,filter_num]
                ...
                pool=tf.nn.max_pool(relu_conv,ksize=[1,max_news_size-temp_filter_size.eval()+1,1,1],
        ...
        total_filter_num=len(tf.unstack(filter_size))*filter_num
        self.pooled_feature=tf.concat(res_after_pool,3) 
        ...
        with tf.name_scope('output'):
            print([total_filter_num,topic_size])
            #w=tf.get_variable('w',shape=[total_filter_num,topic_size],initializer=tf.contrib.layers.xavier_initializer())
            w=tf.Variable(tf.truncated_normal(shape=tf.stack([total_filter_num,topic_size]),stddev=0.1),name='w')
        ...
        with tf.name_scope('loss'):
            losses=tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.input_topic,logits=self.scores)
