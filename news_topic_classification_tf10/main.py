...
tf.flags.DEFINE_integer("epoch_size",2,"Default 10")
tf.flags.DEFINE_integer("batch_size",300,"Default 300")
tf.flags.DEFINE_integer("evaluate_step",20,"Evaluate each 20[default] global steps")
...
print(FLAGS.train_dev_split_ratio*len(y_label))
temp_count=int(math.floor(FLAGS.train_dev_split_ratio*len(y_label)))
x_train=x_nums[:temp_count,:]
y_train=y_label[:temp_count]
x_dev=x_nums[temp_count:,:]
y_dev=y_label[temp_count:]
...
with gra.as_default():
    ...
    with sess.as_default():
        cnn=cnn.topicCNN(vocabulary_size=vocabulary_size,embedding_size=FLAGS.embedding_size,
                         filter_size=tf.stack(map(int,FLAGS.filter_size.split(','))),
        ...
        #directory of output
        out_file='/u/80/shic1/unix/Desktop'
        print('Output Directory: '+out_file)
        
        #Summaries
        loss_summary=tf.summary.scalar('loss',cnn.loss)
        accuracy_summary=tf.summary.scalar('accuracy',cnn.accuracy)
        train_summary_op=tf.summary.merge([loss_summary,accuracy_summary])
        dev_summary_op=tf.summary.merge([accuracy_summary])
        #train_summary_op = tf.summary.merge_all()
        train_summary_dir=os.path.join(out_file,'summary','train')
        dev_summary_dir=os.path.join(out_file,'summary','dev')
        ...
        def train_one_step(x_batch,y_batch):
            ...
            _,summary,step,loss,accuracy=sess.run([train_op,train_summary_op,global_step,cnn.loss,cnn.accuracy],feed_dict)
            train_summary_writer.add_summary(summary,step)
            if step%20==0:
                saver.save(sess,os.path.join(out_file,'summary','checkpoint','model.ckpt'),step)
        ...
        def dev_one_step(x_batch,y_batch):
            ...
            summary,step,loss,accuracy=sess.run([dev_summary_op,global_step,cnn.loss,cnn.accuracy],feed_dict)
